...
public class Sokoban {
    ...
    public void doLoad() {
        ...
        for (int row = 0; row < rows; row++) {
            ...
            for (int col = 0; col < cols; col++) {
                ...
                if (ch == 'w') {
                    ...
                    cells[row][col] = new Cell("wall");
                else if (ch == 's')
                    cells[row][col] = new Cell("shelf");
                else if (ch == 'b')
                    cells[row][col] = new Cell("box");
                else {
        ...
        drawWarehouse();
        UI.printMessage("Level " + level
                + ": Push the boxes to their target positions. Use buttons or put mouse over warehouse and use keys (arrows, wasd, ijkl, u)");
    }
