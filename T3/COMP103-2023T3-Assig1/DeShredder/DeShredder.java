...
public class DeShredder {
    ...
    public void rotateList() {
        if (allShreds.size() > 0) {
            allShreds.add(allShreds.remove(0));
        }
        display();
    }
    ...
    public void doMouse(String action, double x, double y) {
        ...
        if (action.equals("released")) {
            ...
            if (fromStrip == allShreds && toStrip == workingStrip) {
                moveFromAllShredsToWorkingStrip(fromPosition, toPosition);
            }
            if (fromStrip == workingStrip && toStrip == allShreds) {
                moveFromWorkingStripToAllShreds(fromPosition, toPosition);
            }
            if (fromStrip == workingStrip && toStrip == workingStrip) {
                moveWithinWorkingStrip(fromPosition, toPosition);
            }
            display();
        }
    }
    ...
    public void moveFromAllShredsToWorkingStrip(int fromPosition, int toPosition) {
        if (toPosition > workingStrip.size()) {
            workingStrip.add(allShreds.remove(fromPosition));
        } else {
            workingStrip.add(toPosition, allShreds.remove(fromPosition));
        }
    }

    public void moveFromWorkingStripToAllShreds(int fromPosition, int toPosition) {
        if (toPosition > allShreds.size()) {
            allShreds.add(workingStrip.remove(fromPosition));
        } else {
            allShreds.add(toPosition, workingStrip.remove(fromPosition));
        }
    }

    public void moveWithinWorkingStrip(int fromPosition, int toPosition) {
        Shred shred = workingStrip.remove(fromPosition);
        if (toPosition > workingStrip.size()) {
            workingStrip.add(shred);
        } else {
            workingStrip.add(toPosition, shred);
        }
    }
