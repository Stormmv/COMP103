 ...
 * A MazeCell is Iterable, so that you can iterate through its neighbour cells
 * with
 * for(MazeCell neighbour : cell){....
 *
 * The maze has a goal cell (shown in green, two thirds towards the bottom right
 * corner)
...
public class MazeSearch {
    ...
    public void exploreFromCellAll(MazeCell cell) {
        ...
        if (cell == maze.getGoal()) {
            pathCount++;
            UI.println("Found path #" + pathCount);
            cell.draw(Color.blue); // to indicate finding the goal
            UI.sleep(1000);
            return;
        }
    ...
    public void exploreFromCellShortest(MazeCell start) {
        ...
        queue.add(start);
        HashMap<MazeCell, MazeCell> previous = new HashMap<>();
        ...
        while (!queue.isEmpty()) {
            ...
            for (MazeCell neighbour : current) {
    ...
    public void drawShortestPath(HashMap<MazeCell, MazeCell> previous) {
        MazeCell current = maze.getGoal();
        while (current != null) {
            current.draw(Color.green);
            UI.sleep(delay);
