...
public class HospitalERCore{
    ...
    private int totalWaitingTime = 0;   // total waiting time of all patients
    private int totalTreatmentTime = 0; // total treatment time of all patients
    private int numDischargedPatients = 0; // number of patients discharged
    ...
    public void run(){
        ...
        while (running){         // each time step, check whether the simulation should pause.
            ...
            //if patient has finished tretment use 
            time++; 
     ...
     * (Called every tick of the simulation clock)
     */
    public void moveFromWaitingRoom(){
        if (treatmentRoom.size() < MAX_PATIENTS){
            Patient highestPriorityPatient = null;
            for (Patient p : waitingRoom){
                if (highestPriorityPatient == null || p.compareTo(highestPriorityPatient) == -1){
                    highestPriorityPatient = p;
                }
            }
            if (highestPriorityPatient != null){
                waitingRoom.remove(highestPriorityPatient);
                treatmentRoom.add(highestPriorityPatient);
            }
        }
    ...
    /**
     * Move patients from the treatment room to the discharged list if they are
    ...
    public void reportStatistics(){
        UI.println("Number of patients treated: "+ numDischargedPatients);
        UI.println("Average waiting time: "+ (totalWaitingTime/numDischargedPatients));
        UI.println("Average treatment time: "+ (totalTreatmentTime/numDischargedPatients));
    }
